...
import { GEMINI_BROKER_BLOCKER, getGeminiConnectionService } from "@/lib/connections/runtime";
...
export async function GET(request: Request) {
  ...
  const service = await getGeminiConnectionService();
  if (!service) return Response.json({ status: "connection_error", last_verified_at: null, fingerprint: null, last_four: null, error: GEMINI_BROKER_BLOCKER }, { headers });
  try { return Response.json(await service.status(owner), { headers }); }
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    return Response.json(await service.save(owner, body.api_key, body.confirmation), { headers });
  } catch (error) {
...
export async function DELETE(request: Request) {
  ...
  try { return Response.json(await service.disconnect(owner), { headers }); }
