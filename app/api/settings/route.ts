...
export async function GET(request: Request): Promise<Response> {
  ...
  if (!config.url || !config.token) {
    const providers: ProviderStatus[] = [
      { provider: "codex_oauth", status: "blocked", blocker: SETTINGS_BLOCKER },
      { provider: "gemini", status: "blocked", blocker: SETTINGS_BLOCKER },
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (body.provider === "gemini" && (typeof body.api_key !== "string" || body.api_key.trim().length < 20)) {
  ...
  const brokerBody = body.provider === "gemini"
    ? { provider: "gemini", api_key: body.api_key }
